...
import { CASE_SECTIONS, isCaseSection, setCaseSection, subscribeCaseSection, type CaseSectionId } from "@/lib/caseSection";
...
const relevanceRank: Record<Relevance, number> = { HIGH: 0, MEDIUM: 1, LOW: 2 };
...
export default function CaseEntitiesPage() {
  ...
  const [section, setSection] = useState<CaseSectionId>("ai-insight");
  const listRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const apply = (id: string) => {
      if (isCaseSection(id)) setSection(id);
      if (id === "") setSection("ai-insight");
    };
  ...
  return (
    <main className="bg-[#080808] text-neutral-200 lg:flex lg:h-screen lg:flex-col lg:overflow-hidden">
      <div className="mx-auto flex w-full max-w-[1500px] flex-1 flex-col px-6 pt-12 pb-6 md:px-10 lg:min-h-0">
        ...
        <div className="mt-5 grid min-h-[540px] flex-1 grid-rows-[auto] gap-5 lg:min-h-0 lg:grid-cols-[minmax(280px,0.9fr)_minmax(0,1.7fr)]">
          {/* LEFT: entity list */}
